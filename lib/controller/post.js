  ...
  req.info.post.save().then(() => {
    Comment
      ...
      .then((rawComments) => {
        ...
        response.respondSuccess(req, res, makePostInfo(req.info.user._id.toString(), req.info.post, comments));
      }, (err) => {
        logger.error(err);
        response.respondError(req, res, status.codes.CommentsOnPostGottenFailed.code);
      });
...
PostController.prototype.loadPost = function (req, res, next) {
  if (undefined === req.params ||
    ...
    false === sanitizeObjectId(req.params.pid)) {
...
PostController.prototype.updateVoteOnPost = function (req, res) {
  ...
  } else if (curUserIdStr === req.info.post.author._id.toString()) {
