...
PostController.prototype.getPost = function (req, res) {
  ...
  // 	    };
  // 	    response.respondSuccess(req, res, gotPost._id.toString());
  //    	}
  // });
  let gotPost = {
    ...
    votes: req.info.post.votes,
    comments: req.info.post.comments
  };
  response.respondSuccess(req, res, gotPost._id.toString());
};
...
PostController.prototype.addPost = function (req, res) {
  ...
  if (regExCheckEmptyText.test(newPost.title) || newPost.title === undefined) {
  ...
  if (regExCheckEmptyText.test(newPost.text) || newPost.text === undefined) {
...
PostController.prototype.updateVoteOnPost = function (req, res) {
  ...
  var newVoteUid = req.body.vote.uid;

  // console.log(typeof(newVoteUid));
  // console.log(typeof(req.info.post.author));
  if (("\"" + newVoteUid + "\"") === JSON.stringify(req.info.post.author)) {
    response.respondError(req, res, status.codes.PostOwnerCanNotVote.code);
    return;
  }

  for (var i = 0; i < req.info.post.votes.length; i++) {
    //  what is the better way?
    if (JSON.stringify(req.info.post.votes[i].uid) === ("\"" + newVoteUid + "\"")) {
  ...
  Post
    .update({ _id: req.params.pid }
      , { $push: { votes: { uid: newVoteUid, voteType: newVoteType } } }
      )
