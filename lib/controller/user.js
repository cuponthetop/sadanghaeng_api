  ...
  if (true === isLoggedIn(req)) {
    ...
    generateResetTokenWorker(email).then((user) => {
      ...
      if (process.NODE_ENV !== 'test') {
        // TODO: 이거 하이퍼 링크로 보내게
        mailer.sendMail(user.email, '가림 비밀번호 변경', '비밀번호 변경을 위한 링크는 ' + user.reset_token + ' 입니다.');
      }
...
UserController.prototype.generateVerifyToken = function (req, res) {
  if (true === isLoggedIn(req)) {
    ...
    generateVerifyTokenWorker(email).then((user) => {
      ...
      if (process.NODE_ENV !== 'test') {
        // TODO: 이거 하이퍼 링크로 보내게
        mailer.sendMail(user.email, '가림 이메일 인증', '이메일 주소 인증을 위한 링크는 ' + user.verify_token + ' 입니다.');
      }
