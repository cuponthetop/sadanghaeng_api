...
var isOwner = function (req) {
  ...
  if (req.params !== undefined &&
    ...
    if (req.info.post.author._id !== undefined) {
      ownerId = req.info.post.author._id.toString();
    } else {
      ownerId = req.info.post.author.toString();
    }
  } else if (req.info.comment !== undefined) {
    if (req.info.comment.author._id !== undefined) {
      ownerId = req.info.comment.author._id.toString();
    } else {
        ...
        case 401:
          agent
            ...
            .catch((err2) => {
              ...
              response.respondError(req, res, status.codes.UnknownError, err2);
            })
            .done();
          break;
        default:
          ...
          response.respondError(req, res, status.codes.UnknownError, err);
          break;
      }
    })
    .done();
...
var getUserFromEmail = function (email) {
  var deferred = Q.defer();
...
UserController.prototype.login = function (req, res) {
  ...
  if (false === UnivCtrl.isValidEmailAddress(req.body.email)) {
      ...
      .then((user) => {
        agent
          ...
          .then((innerResponse) => {
            ...
            response.respondSuccess(req, res, { userId: user._id.toString(), univId: user.university.toString() });
          })
          .catch((err) => {
            if (undefined !== req.session) {
              req.session.destroy();
...
UserController.prototype.generateResetToken = function (req, res) {
  if (true === isLoggedIn(req)) {
    response.respondError(req, res, status.codes.UserAlreadyLoggedIn.code);
  } else if (false === UnivCtrl.isValidEmailAddress(req.query.email)) {
    response.respondError(req, res, status.codes.InvalidEmailAddress.code);
  } else {
    ...
    if (null === user) {
      ...
      UnivCtrl.getUniversityFromEmail(fields.email).then((university) => {
        if (null !== university) {
          ...
          user.university = university._id.toString();
          user.nickname = university.displayName;
          user.save().then((savedUser) => {
