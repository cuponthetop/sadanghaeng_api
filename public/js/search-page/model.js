(function (window) {
   ...
   */
	...
	Model.prototype.getSearchResultList = function(parameter, callback) {
		HttpUtil.get(HOST_URL + '/api/v1/universities/' + parameter.univid + '/search', parameter, function (err, result) {
			if (err) {
				return callback([]);
			}
			if (result && result.status === 0) {
